...
const STATE_KEY = "vimariTabState";
...
const stateReady = initializeState().catch(error => {
    console.error("Unable to initialize Vimari tab state:", error);
});
...
if (browser.tabs.onActivated) browser.tabs.onActivated.addListener(activeInfo => {
    recordActivation(activeInfo).catch(error => console.error("Unable to store Vimari tab activation:", error));
});
if (browser.tabs.onRemoved) browser.tabs.onRemoved.addListener((tabId, removeInfo) => {
    ...
    cacheClosedTab(snapshot).catch(error => console.error("Unable to cache a closed Vimari tab:", error));
});
